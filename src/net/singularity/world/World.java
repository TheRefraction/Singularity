...
import net.singularity.Singularity;
...
public class World {
    ...
    private final ArrayList<WorldListener> worldListeners = new ArrayList();
    public byte[] blocks;
    ...
    public World(Camera camera, int width, int depth, int height) {
        ...
        if(!mapLoaded) {
            this.blocks = (new WorldGen(width, depth, height)).generateMap();
        }
    ...
    public boolean load() {
        try {
            ...
            for (WorldListener worldListener : this.worldListeners) {
                worldListener.allChanged();
            }

            dis.close();
            return true;
        } catch (Exception ex) {
            Logger.getLogger(Singularity.class.getName()).log(Level.WARNING, null, ex);
            return false;
        }
    }
        ...
        for(int x = x0; x < x0 + x1; ++x) {
            for(int z = y0; z < y0 + y1; ++z) {
                ...
                if (oldDepth != y) {
                    ...
                    for (WorldListener worldListener : this.worldListeners)
                        worldListener.lightColumnChanged(x, z, yl0, yl1);
                }
            }
        }
    }

    public void addListener(WorldListener levelListener) {
        this.worldListeners.add(levelListener);
    }

    public void removeListener(WorldListener levelListener) {
        this.worldListeners.remove(levelListener);
    }
    ...
    public boolean isFree(AABB aabb) {
        //check for entities aabb here
        return !this.player.aabb.intersects(aabb);
    }
    ...
    public boolean setTile(int x, int y, int z, int type) {
        if (x >= 0 && y >= 0 && z >= 0 && x < this.width && y < this.depth && z < this.height) {
            if (type == this.blocks[(y * this.height + z) * this.width + x]) {
                ...
                for (WorldListener worldListener : this.worldListeners) {
                    worldListener.tileChanged(x, y, z);
                }
    ...
    public ArrayList<AABB> getCubes(AABB aABB) {
        return getCubes(aABB, false);
    }

    public ArrayList<AABB> getCubes(AABB aABB, boolean pick) {
            ...
            for(int y = y0; y < y1; ++y) {
                for(int z = z0; z < z1; ++z) {
                    ...
                    if (block != null) {
                        ...
                        if ((block.isSolid() || pick) && aabb != null) {
                            aabbList.add(aabb);
                        }
                    }
                }
